...
def phase1():
    ...
    activity_level = request.args.get('activity_level', type=int)
    if None in (gender, weight, height, age, activity_level):
        return jsonify({"error": "Missing parameters"}), 400
    calories_tobe_burnt = calculate_calories_burnt(weight, height, age, gender, activity_level)
    print(calories_tobe_burnt)
    exercise_todo = balanced_exercise_predictor(calories_tobe_burnt, weight)
    print(exercise_todo)
    # print(exercise_todo)
    response = {
        "calToBurnt": calories_tobe_burnt,
        "exToDo": exercise_todo,
    }
...
def phase2():
    ...
    heart_rate = request.args.get('heart_rate', type=float)
    body_temp = request.args.get('body_temp', type=float)
...
def processed_img(img_path_or_url):
    if img_path_or_url.startswith('http://') or img_path_or_url.startswith('https://') or True:
